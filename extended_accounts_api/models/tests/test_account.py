...
class AccountModelTestCase(TestCase):
    ...
    def setUpClass(cls, *args, **kwargs):
        ...
        cls.account = Account.objects.create_user(**cls.data)
    ...
    def test_create_user_OK(self):
        self.assertEqual(self.account.username, self.data["username"])
        self.assertEqual(self.account.email, self.data["email"])
        ...
        self.assertEqual(self.account.profile.first_name, self.data["first_name"])
        self.assertEqual(self.account.profile.last_name, self.data["last_name"])
        self.assertEqual(self.account.profile.phone_number, self.data["phone_number"])
        self.assertIn(
    ...
    def test_update_user_KO_if_wrong_data(self):
        ...
        with self.assertRaises(IntegrityError):
            self.account.update(username="jdoe")
        self.assertEqual(
            self.account.username, self.data["username"]
    ...
    def test_manager_with_perm_OK(self):
        ...
        users_with_perm = Account.objects.with_perm(
            self.permission,
            backend="extended_accounts_api.models.tests.test_account.UselessBackend",
        )
        self.assertNotIn(self.account, users_without_perm)
